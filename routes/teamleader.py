...
def teamleader_dashboard():
    ...
    current_week_no = (
        (today - project_start.date()).days // 7
    ) + 1
    ...
    case(
        ...
        # 1️⃣ Survey Form YES + Raw Video YES + Final Report NO → TOP
        (
            and_(
        ...
        # 2️⃣ All three YES → BOTTOM (but before cancelled)
        (
            and_(
                Survey.survey_form_completed == True,
                Survey.task1_completed == True,
        ...
        # 3️⃣ CANCELLED → ABSOLUTE LAST
        (
            Survey.status == "cancelled",
            4
        ),

        # 4️⃣ Everything else → MIDDLE
        else_=2
    ),
